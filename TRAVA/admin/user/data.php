...
$allowed_sort = ['id','nama','email','created_at'];
...
$where = $search ? "WHERE role='user' AND (nama LIKE '%$search%' OR email LIKE '%$search%')" : "WHERE role='user'";
$total_rows = mysqli_fetch_assoc(mysqli_query($conn,"SELECT COUNT(*) as c FROM users $where"))['c'];
$total_pages = max(1, ceil($total_rows / $per_page));
$page = min($page, $total_pages);
$offset = ($page - 1) * $per_page;

$data = mysqli_query($conn, "SELECT * FROM users $where ORDER BY $sort_col $sort_dir LIMIT $per_page OFFSET $offset");

function sort_link_u($col, $label, $cc, $cd, $s, $pp) {
...
<title>Data Pengguna — TRAVA Admin</title>
...
:root{--sidebar:#1a3a5c;--sidebar-hover:rgba(255,255,255,0.07);--sidebar-active:rgba(255,255,255,0.13);--sidebar-border:rgba(255,255,255,0.1);--sidebar-w:264px;--page:#f0f2f5;--page-alt:#e4e7eb;--card:#ffffff;--border:rgba(0,0,0,0.08);--shadow:0 2px 8px rgba(0,0,0,0.06);--tx-1:#1a202c;--tx-2:#4a5568;--tx-3:#a0aec0;--terra:#c0522a;--gold:#b08430;--forest:#3a6b45;--ocean:#1a5c7a;--s-tx:#d6e8f5;--s-muted:#7fa4c2;}
...
<aside class="sidebar">
  ...
  <a href="../user/data.php" class="nav-item active"><i class="fa-solid fa-users"></i>Data Pengguna</a>
...
<main class="main">
  <div class="topbar">
    <div class="topbar-left">
      <div class="page-title">Data <span>Pengguna</span></div>
    </div>
  ...
  <form method="GET" id="mainForm">
    ...
    <!-- Row 2: Show entries (left) + Search (right) — no Tambah for user page -->
  ...
  <div class="table-wrap">
    <table>
      <thead>
        <tr>
          <th>No</th>
          <th><?= sort_link_u('nama','Nama',$sort_col,$sort_dir,$search,$per_page) ?></th>
          <th><?= sort_link_u('email','Email',$sort_col,$sort_dir,$search,$per_page) ?></th>
          <th><?= sort_link_u('created_at','Terdaftar',$sort_col,$sort_dir,$search,$per_page) ?></th>
          <th>Aksi</th>
        </tr>
      </thead>
      <tbody>
      <?php if(mysqli_num_rows($data)===0): ?>
        <tr><td colspan="5"><div class="empty-state"><i class="fa-solid fa-users"></i>Tidak ada data pengguna ditemukan.</div></td></tr>
      <?php else: $no=$offset+1; while($d=mysqli_fetch_assoc($data)): ?>
        <tr>
          ...
          <td>
            <a href="hapus.php?id=<?= $d['id'] ?>" class="btn-hapus" onclick="return confirm('Hapus pengguna ini?')"><i class="fa-solid fa-trash"></i> Hapus</a>
          </td>
        </tr>
      <?php endwhile; endif; ?>
      </tbody>

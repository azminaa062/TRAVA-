...
$allowed_sort = ['id','rating','created_at'];
...
// Fixed: use correct FK column names from SQL schema (user_id, wisata_id)
$join = "FROM review r LEFT JOIN users u ON r.user_id = u.id LEFT JOIN wisata w ON r.wisata_id = w.id";
$where = $search ? "WHERE u.nama LIKE '%$search%' OR w.nama LIKE '%$search%' OR r.komentar LIKE '%$search%'" : '';
$total_rows = mysqli_fetch_assoc(mysqli_query($conn,"SELECT COUNT(*) as c $join $where"))['c'];
$total_pages = max(1, ceil($total_rows / $per_page));
$page = min($page, $total_pages);
$offset = ($page - 1) * $per_page;

$data = mysqli_query($conn, "SELECT r.*, u.nama as nama_user, w.nama as nama_wisata $join $where ORDER BY r.$sort_col $sort_dir LIMIT $per_page OFFSET $offset");

function sort_link_r($col, $label, $cc, $cd, $s, $pp) {
...
<title>Data Review — TRAVA Admin</title>
...
.show-entries select{padding:7px 10px;border:1.5px solid var(--border);border-radius:8px;background:var(--card);font-family:'Lato',sans-serif;font-size:13px;color:var(--tx-1);outline:none;cursor:pointer;}
...
.search-wrap input{width:240px;padding:9px 14px 9px 36px;border:1.5px solid var(--border);border-radius:8px;background:var(--card);font-family:'Lato',sans-serif;font-size:13px;color:var(--tx-1);outline:none;transition:border-color 0.18s;}
...
<aside class="sidebar">
  ...
  <a href="../review/data.php" class="nav-item active"><i class="fa-solid fa-star"></i> Data Review</a>
...
<main class="main">
  <div class="topbar">
    <div class="topbar-left">
      <div class="page-title">Data <span>Review</span></div>
    </div>
    ...
    <div class="toolbar-row2">
      ...
      <div class="toolbar-row2-right">
        <div class="search-wrap">
          ...
          <input type="text" name="search" placeholder="Cari pengguna, destinasi, komentar..." value="<?= htmlspecialchars($search) ?>">
        </div>
  ...
  <div class="table-wrap">
    <table>
      <thead>
        <tr>
          ...
          <th>Pengguna</th>
          <th>Destinasi</th>
          <th><?= sort_link_r('rating','Rating',$sort_col,$sort_dir,$search,$per_page) ?></th>
          <th>Komentar</th>
          <th><?= sort_link_r('created_at','Tanggal',$sort_col,$sort_dir,$search,$per_page) ?></th>
          <th>Aksi</th>
        </tr>
      </thead>
      <tbody>
      <?php if(mysqli_num_rows($data)===0): ?>
        <tr><td colspan="7"><div class="empty-state"><i class="fa-solid fa-star"></i>Tidak ada data review ditemukan.</div></td></tr>
      <?php else: $no=$offset+1; while($d=mysqli_fetch_assoc($data)): ?>
        <tr>
          ...
          <td><span class="rating-cell"><i class="fa-solid fa-star" style="color:var(--gold);font-size:12px;"></i><?= $d['rating'] ?></span></td>
          <td style="max-width:260px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= htmlspecialchars($d['komentar'] ?? '') ?></td>
          <td><?= isset($d['created_at']) ? date('d M Y', strtotime($d['created_at'])) : '-' ?></td>
          <td>
            <a href="hapus.php?id=<?= $d['id'] ?>" class="btn-hapus" onclick="return confirm('Hapus review ini?')"><i class="fa-solid fa-trash"></i> Hapus</a>
          </td>
        </tr>
      <?php endwhile; endif; ?>
      </tbody>
    ...
    <div class="pagination-wrap">
      ...
      <div class="pagination">
        <?php if($page>1): ?>
          <a href="?<?= http_build_query(array_merge($qb,['page'=>$page-1])) ?>" class="prev-next"><i class="fa-solid fa-chevron-left" style="font-size:10px;"></i> Prev</a>
        <?php else: ?>
        ...
        <?php if($page<$total_pages): ?>
          <a href="?<?= http_build_query(array_merge($qb,['page'=>$page+1])) ?>" class="prev-next">Next <i class="fa-solid fa-chevron-right" style="font-size:10px;"></i></a>
        <?php else: ?>
